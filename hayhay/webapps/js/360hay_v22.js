...
function showWebcontent(url, isIs360hayArticle,articleId,cateName, ref) {
	...
	if(url.indexOf("www.youtube.com") > 0){
		...
		loadYoutubeArticles();
	}else if(url.indexOf("muabannhadat.vn") > 0 || url.indexOf("phimmoi.net")  > 0 || url.indexOf("tienphong.vn")  > 0 || url.indexOf("foxnews.com")  > 0  || url.indexOf("nytimes.com")  > 0 ){
		window.open(url);
...
function renderNewsHTML(fromPage,fromNewsIndex, categories,cate){
	...
	View.renderHTML("newsContainer",data, "/js/ejs/templates/globalnewsindex_news_v3.ejs", true)
	//swiper();
