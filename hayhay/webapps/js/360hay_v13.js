...
function showWebcontent(url, isIs360hayArticle,articleId,cateName, ref) {
	...
	if(url.indexOf("www.youtube.com") > 0){
		...
		loadYoutubeArticles();
	}else if(url.indexOf("muabannhadat.vn") > 0 || url.indexOf("phimmoi.net") > 0 || (fromApp != null && fromApp != undefined && fromApp == "true" )){
		window.open(url);
	}else{
		if(ref == undefined)
			ref = "";
...
function renderNewsHTML(fromPage,fromNewsIndex, categories,cate){
	...
	var images = cates[0].newsImages;
	var data ={categoriesNews:cates,from:fromPage,fromIndex:fromNewsIndex};
	View.renderHTML("newsContainer",data, "/js/ejs/templates/groupnewsbycategory_v5.ejs", true)
	swiper();
	loadImages(images);
			...
			View.scroll(View.scrollPosition._35_PERCENT,function(){
				...
				while(renderCount > 0){
					if(currentIndex < result.categories.length){
						...
						View.renderHTML("newsContainer",data, "/js/ejs/templates/groupnewsbycategory_v5.ejs", true)
						swiper();
						loadImages(renderCate[0].newsImages);
					}
					renderCount = renderCount -1;
				}
			});	
